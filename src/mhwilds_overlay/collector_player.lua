...
function (args)
    ...
    if skill.get_Status == nil then return end
    local ok_status, status = pcall(function()
        return skill:get_Status()
    end)
    if (not ok_status) or (not status) then
        if _G.__MHWS_LOG_ERROR ~= nil then
            _G.__MHWS_LOG_ERROR("mhwilds_overlay/collector_player.lua:get_Status", status)
        end
        return
    end

    if status.get_IsMaster == nil then return end
    local ok_master, is_master = pcall(function()
        return status:get_IsMaster()
    end)
    if (not ok_master) then
        if _G.__MHWS_LOG_ERROR ~= nil then
            _G.__MHWS_LOG_ERROR("mhwilds_overlay/collector_player.lua:get_IsMaster", is_master)
        end
        return
    end
